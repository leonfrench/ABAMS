...
public class LoadHumanCSV {
	...
	public LoadHumanCSV(String ID, boolean commonRegions) throws Exception {
		...
		try {
			...
			String filename = "";
			if (ID.equals("9861"))
				filename = "C:\\Users\\leon\\Desktop\\Human array\\" + ID
						+ "\\matrix.29192 x 323.txt";
			if (ID.equals("10021"))
				filename = "C:\\Users\\leon\\Desktop\\Human array\\" + ID
						+ "\\matrix.29192 x 346.txt";

			DoubleMatrix<String, String> inputMatrix = reader.read(filename);
			matrix = new ABAMSDataMatrix(inputMatrix, "HumanArray",
					new CorrelationAdjacency(inputMatrix));
	...
	public Map<String, String> getAcroToNameMap() throws Exception {
		...
		// "C:\\Users\\leon\\Desktop\\Human array\\" + ID + "\\SampleAnnot.csv"
		// );
		FileReader fr = new FileReader(
				"/home/leon/Desktop/Windows Desktop/Stuff from Japan/Human array/"
						+ ID + "/SampleAnnot.csv");
	...
	public void makeMatrix() throws Exception {
		...
		FileReader fr = new FileReader(
				"C:\\Users\\leon\\Desktop\\Human array\\" + ID
						+ "\\SampleAnnot.csv");
		CSVReader reader = new CSVReader(fr);
		// eat header
		reader.readNext();
		...
		fr = new FileReader("C:\\Users\\leon\\Desktop\\Human array\\" + ID
				+ "\\Probes.csv");
		...
		LinkedList<String> geneNames = new LinkedList<String>(
				new HashSet<String>(probesToGene.values()));
		LinkedList<String> regionNamesInMatrix = new LinkedList<String>(
				new HashSet<String>(regionNames));
		DoubleMatrix<String, String> matrix = new DenseDoubleMatrix<String, String>(
				geneNames.size(), regionNamesInMatrix.size());
		...
		while ((line = reader.readNext()) != null) {
			...
			for (int i = 1; i < line.length; i++) {
				...
				matrix.setByKeys(gene, regionName,
		...
		Util.writeRTable("C:\\Users\\leon\\Desktop\\Human array\\" + ID
				+ "\\matrix." + matrix.rows() + " x " + matrix.columns()
				+ ".txt", matrix);
	}

	// //////////////////////////////////////////////////
	public static void makeMNIMatrix(String ID) throws Exception {
		LinkedList<String> regionNames = new LinkedList<String>();
		// /home/abams/Desktop/human/9861
		String prefix = "/home/abams/Desktop/human/";
		FileReader fr = new FileReader(prefix + ID + "/SampleAnnot.csv");
		CSVReader reader = new CSVReader(fr);
		// eat header
		reader.readNext();
		String line[];
		...
		while ((line = reader.readNext()) != null) {
			log.info(line[5]);
			String region;
			log.info("x=" + line[10] + " y=" + line[11] + " z=" + line[12]);
			region = "(" + line[10] + "," + line[11] + "," + line[12] + ")";
//			System.exit(1);
			regionNames.addLast(region);
			samplesPerRegion.increment(region);
		}
		...
		// CountingMap<Integer> probesPerGene= new CountingMap<Integer>();
		Map<String, String> probesToGene = new HashMap<String, String>();
		fr = new FileReader(prefix + ID + "/Probes.csv");
		reader = new CSVReader(fr);
		while ((line = reader.readNext()) != null) {
			String probe = line[0];
			String gene = line[3];
		...
		fr = new FileReader(prefix + ID + "/MicroarrayExpression.csv");
		reader = new CSVReader(fr);
		int lineCount = 0;
		while ((line = reader.readNext()) != null) {
			if (lineCount++ % 1000 == 0)
		...
		log.info(matrix.rows() + " x " + matrix.columns());
		Util.writeRTable(prefix + ID + "/matrix.MNI(xyz)." + matrix.rows()
				+ " x " + matrix.columns() + ".txt", matrix);
	}
	...
	public void test(int size1, int size2, double statistic, boolean coronal)
			throws Exception {
		...
		for (int i = 0; i < 10000; i++) {
			...
			if (sampleStatistic < statistic) {
				lesserHits++;
				log.info("Sample " + "Spearman correlation" + ":"
						+ sampleStatistic + " less than " + statistic);
			}
			if (sampleStatistic > statistic) {
				greaterHits++;
				// log.info( "Sample " + "Spearman correlation" + ":" +
				// sampleStatistic + " greater than " + statistic
				// );
			}
		}
		double mean = DescriptiveWithMissing.mean(sampleHistory);
		String statisticString = "Spearman";
		log.info("Sample " + statisticString + " greater than " + statistic
				+ ", " + greaterHits + " times");
		log.info("Sample " + statisticString + " less than " + statistic + ", "
				+ lesserHits + " times");
		log.info("average resample" + statisticString + " " + mean + "");
		log.info("standard deviation resample"
				+ statisticString
				+ " "
				+ Math.sqrt(DescriptiveWithMissing.sampleVariance(
						sampleHistory, mean)));
	...
	public static void missingRegions() throws Exception {
		...
		FileTools.stringsToFile((Set<String>) Util.intersect(h10021, h9861),
				base);
	...
	public static void forJoao(String ID) throws Exception {
		...
		ABAMSDataMatrix toWrite = human
				.slimToRows(new HashSet<String>(JoaoIDs));
	...
	public static void main(String[] args) throws Exception {
		...
		makeMNIMatrix(IDD);
		log.info("DONE!");
		// LoadHumanCSV mni = new LoadHumanCSV( IDD, false );
		...
		human.test(NEmatrix.rows(), OEmatrix.rows(),
				Util.spearmanCorrel(NEarray, OEarray), coronal);
