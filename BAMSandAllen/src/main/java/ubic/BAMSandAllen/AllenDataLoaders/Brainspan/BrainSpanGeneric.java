...
public class BrainSpanGeneric {
	...
	public static void createCorrelationMatrix(BrainSpanGeneric loader,
			DoubleMatrix<String, String> inputMatrix, Set<String> geneGroup,
			String name) throws Exception {
		DoubleMatrix<String, String> resultMatrix = new DenseDoubleMatrix<String, String>(
				geneGroup.size(), inputMatrix.columns());
		resultMatrix.setColumnNames(inputMatrix.getColNames());
		resultMatrix.setRowNames(new LinkedList<String>(geneGroup));
		for (String testGene : geneGroup) {
			...
			DoubleMatrix<String, String> corResult = loader.correlateViaTime(
					inputMatrix, testGeneData);

			for (String colName : corResult.getColNames()) {
				resultMatrix.setByKeys(testGene, colName,
						corResult.getByKeys("Spearman", colName));
			}

		}
		String filename = "/home/abams/Desktop/human/BrainSpan/GENE/correlations/"
				+ name + ".tsv";
		Util.writeRTable(filename, resultMatrix);
	}
